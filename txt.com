...
def liveresult():
    ...
    for i in range (0,devicecount):
        ...
        return render_template('inventory.html', liveresult= foutput)
...
def livef5result():
    ...
    if action == "Software available and active version":
        url = 'https://'+ devicename + '/mgmt/tm/sys/software/volume'
        payload = ""
        headers = {
 ...
 #      response = requests.request("GET", url, headers=headers, data=payload, verify=False)
  ...
  #  return render_template('inventory.html', livef5result= response.json())
    ...
    return render_template('inventory.html', livef5result= json.dumps(response,indent=2))


@app.route('/livefmcresult', methods = ["POST", "GET"])
def livefmcresult():
    inputitems = request.form.to_dict()
    devicename = inputitems['Devicename']
    username = inputitems['Username']
    password = inputitems['password']
    action = inputitems['commandsfmc']
    api_uri = "/api/fmc_platform/v1/auth/generatetoken"
    url = "https://" + devicename + api_uri
    response = requests.post(url,verify=False,auth=HTTPBasicAuth(username, password))
    output = {"X-auth-access-token": response.headers["X-auth-access-token"],"X-auth-refresh-token": response.headers["X-auth-refresh-token"],"DOMAIN_UUID": response.headers["DOMAIN_UUID"]}
    if action == "FMC Version":
        api_uri = "/api/fmc_platform/v1/info/serverversion"
        url = "https://" + devicename + api_uri
        headers = {"X-auth-access-token": output["X-auth-access-token"]}
        response = requests.get(url, verify=False, headers=headers).json()
        database = {}
        devicemodel = str(response["items"][0]['type'])
        softwareversion = str(response["items"][0]['serverVersion'])
        database[devicename] = [devicemodel, softwareversion]
    elif action == "FTD LIST":
        api_uri = "/api/fmc_config/v1/domain/" + output["DOMAIN_UUID"]+"/devices/devicerecords?expanded=true"
        url = "https://" + devicename + api_uri
        headers = {"X-auth-access-token": output["X-auth-access-token"]}
        response = requests.get(url, verify=False, headers=headers).json()
        #output = response.json()
        #devicemodel = str(response["items"][1]['model'])
        database = {}
        leng = int(len(response["items"]))
        for i in range (0, leng):
           devicename = str(response["items"][i]['name'])
           devicemodel = str(response["items"][i]['model'])
           softwareversion = str(response["items"][i]['sw_version'])
           database[devicename] = [devicemodel, softwareversion]
        


    return render_template('inventory.html', livefmcresult= database)
